...
class ZefyrEmbedContainer extends StatefulWidget {
  const ZefyrEmbedContainer({Key key, @required this.node, this.child})
      : super(key: key);

  final EmbedNode node;
  final Widget child;
  @override
  _ZefyrEmbedContainerState createState() => _ZefyrEmbedContainerState();
}

class _ZefyrEmbedContainerState extends State<ZefyrEmbedContainer> {
...
class _EditableEmbed extends SingleChildRenderObjectWidget {
  _EditableEmbed({@required Widget child, @required this.node})
      : assert(node != null),
        super(child: child);
  ...
  RenderEditableEmbed createRenderObject(BuildContext context) {
    return new RenderEditableEmbed(node: node);
  }
